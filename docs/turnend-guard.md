...
Harness-specific tracked hook files adapt each integrated harness's real turn-end mechanism to that shared predicate.
...
- `cursor`: no primary turn-end hook capability has been empirically verified, so Cursor has no tracked integration and its primary protocol relies on bounded foreground checkpoints plus the pull-based `bin/fm-guard.sh` warning.
...
The five tracked harness integrations were validated on 2026-07-08 in scratch repos or throwaway homes, not against the captain's live primary fleet state.
