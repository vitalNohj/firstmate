...
`fm-home-seed.sh` provisions a local isolated home, clones the listed PR-based projects into it, initializes newly cloned `no-mistakes` projects, renders the charter at `data/charter.md`, and `fm-spawn.sh --secondmate` launches it through the same session-provider and status-file path as any direct report.
...
Every generated brief and charter is self-locating: it states its own absolute path and instructs the worker to re-read that file after a context reset or compaction and whenever task intent is uncertain, then reconcile it with that variant's durable progress record before resuming.
This exists because the brief text reaches most harnesses only as the launch prompt, which the harness's own compaction owns and may summarize away, and because `bin/fm-sessionstart-run.sh` deliberately scopes the primary's re-emit out of task worktrees.
The recovery wording binds the re-read to durable evidence (branch commits, the scout report, the secondmate home's own files, plus appended status lines) so recovery resumes rather than restarting completed work.
A seeded secondmate is launched from its home's `data/charter.md` copy, and local or remote provisioning rewrites the scaffold's owned recovery slot so that durable copy names only its own absolute path on its host.
