...
That block owns the live wait shape for the running primary harness: Claude and Grok use background-notify cycles, Codex and Cursor use bounded foreground checkpoints, Pi uses its generated primary watcher extension, and OpenCode uses its TUI plugin.
