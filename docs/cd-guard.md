...
| Cursor | none verified | Cursor has no empirically verified primary hook integration, so the cd-guard is not wired for Cursor. |

Each wired harness runs the cd-guard alongside the watcher-arm seatbelt; the two are independent checks, and either deny blocks the command.
