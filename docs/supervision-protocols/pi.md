...
2. Confirm the Pi primary auto-loaded all three project extensions (plain `pi` or `pi-signed`, after approving project trust once per clone); if not, restart the selected executable with `-e __FM_PI_TURNEND_EXT__ -e __FM_PI_EXT__ -e __FM_PI_ROUTER_EXT__` as a trust-free fallback.
...
The captain-message router extension lives at `__FM_PI_ROUTER_EXT__`.
All three are tracked, project-local `.pi/extensions/*.ts` files that Pi auto-discovers once the project is trusted; `bin/fm-session-start.sh` includes all three in the trust-free restart command.
