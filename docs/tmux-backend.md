...
### Known gap: bare `node` cannot be confidently attributed to Pi or Cursor
...
Cursor Agent can also present as bare `node`, so neither harness can be attributed from that foreground name without deeper and fragile argument inspection.
...
Practical effect: a dead Pi or Cursor secondmate whose pane reports bare `node` is not auto-healed by the liveness sweep today; it is reported as `skipped: liveness probe inconclusive` instead, which still surfaces it for a human to act on.
...
The agent-liveness probe above has one known gap (Pi and Cursor can both present as a generic `node` process name, see above).
