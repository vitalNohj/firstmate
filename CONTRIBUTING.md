...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.pi-lens.json`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  The root `.pi-lens.json` is tracked fleet-wide policy that disables agent-side autoformatting for this repo, because an agent formatter reformatting these bash scripts buries real changes under whitespace-only diffs; it is a shared contract, not one operator's local preference.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.pi-lens.json`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
