...
Codex, OpenCode, and Cursor are also verified and supported as primary harnesses; Codex and Cursor use bounded foreground checkpoints, and OpenCode uses a TUI plugin, so all three carry more harness-specific supervision tradeoffs than the three co-primaries.
Cursor has no empirically verified primary turn-end hook, so its primary protocol also relies on the pull-based next-command warning.
