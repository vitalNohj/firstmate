...
Verified crewmate and scout adapter names are `claude`, `codex`, `opencode`, `pi`, `omp`, `grok`, and `cursor`.
OMP is not verified as a primary or secondmate harness.
...
On every verified primary harness except `cursor`, "no turn ends blind" has a structural backstop beyond the pull-based banner: `bin/fm-turnend-guard.sh` blocks the turn end (or forces one bounded follow-up on passive harnesses) when tasks are in flight without a live identity-matched watcher lock and fresh beacon, guards both the main primary and a secondmate's own primary session, and stays silent when supervision is healthy.
Cursor has no empirically verified primary turn-end hook, so a Cursor primary relies on its bounded foreground checkpoint protocol plus the pull-based `fm-guard.sh` banner.
