...
Shared tracked material is `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.pi-lens.json`, `.github/workflows/`, `bin/`, `.agents/skills/`, and public `skills/`.
...
.pi-lens.json        tracked fleet-wide policy disabling agent-side autoformatting of this repo's scripts (CONTRIBUTING.md "Repo conventions")
...
Every generated brief and charter also states its own absolute path and the reconcile-before-resuming recovery contract; keep both when adjusting generated sections, because a compacted worker has no other route back to its task.
