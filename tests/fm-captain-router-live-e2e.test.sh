...
# isolated home, so the owner launches the configured Pi model itself and the
# verdict must come back with confidence=model.
#
# Set FM_CAPTAIN_ROUTER_MODEL to pin a model this machine is authenticated for
# (the built-in default is cursor/grok-4.6, which needs a Cursor subscription).
set -u
...
if [ -n "${FM_CAPTAIN_ROUTER_MODEL:-}" ]; then
  printf '{ "roles": { "continuity": { "harness": "pi", "model": "%s", "effort": "low" } } }\n' \
    "$FM_CAPTAIN_ROUTER_MODEL" >"$HOME_DIR/config/crew-dispatch.json"
fi
