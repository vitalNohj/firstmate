...
// Words that may lead a node without being the executed command. The shared
// splitter emits `do bin/fm-send.sh ...` as one node, so the fleet script sits
// after the reserved word rather than in the command position.
...
// A reserved word never executes the fleet script that follows it, so advance
// the command position past leading reserved words and any wrapper or
// assignment prefix that then applies to the real command.
function skipReservedLeaders(position, tokens) {
	let current = position;
	while (
		current.command &&
		RESERVED_LEADERS.has(basename(current.command.value))
	) {
		const remaining = current.words.slice(current.index + 1);
		if (remaining.length === 0) return current;
		const next = commandPosition(remaining.map((word) => ({ ...word })));
		if (!next.command) return current;
		const offset = current.index + 1;
		current = {
			...next,
			words: current.words,
			index: offset + next.index,
			command: current.words[offset + next.index],
			wrapperPayloads: [...current.wrapperPayloads, ...next.wrapperPayloads],
		};
	}
	return current;
}
...
function collectExecutedFleetScripts(command, root, depth = 0) {
	...
	for (const tokens of program.nodes) {
		const position = skipReservedLeaders(commandPosition(tokens), tokens);
		...
		for (const payload of position.wrapperPayloads) {
			scripts.push(...collectExecutedFleetScripts(payload, root, depth + 1));
		}
